...
- **高保真轨道动力学:** 
    - 底层调用C++编译的 `libOrbit.so` 动态库进行高精度轨道外推（HPOP）和坐标系转换（ECI to LVLH），确保了环境的物理真实性。
    - 支持J2摄动等复杂动力学模型。
- **部分可观测环境 (POMDP):** 
    - **不完全观测:** 追击方只能在特定时间间隔（`obs_interval`）获得目标的精确位置，模拟了地面雷达的非连续观测特性。对于非观测时刻，智能体依赖其记忆中的“最后已知位置”。
    - **历史依赖:** 智能体必须依赖历史观测序列来推断目标的当前状态和意图。
    - **精细化观测空间:** 为了更好地进行态势感知，追击方的观测信息经过精心设计，包括：自身状态（含燃料）、队友相对状态（LVLH坐标系）、目标的最后已知位置（LVLH坐标系），以及关键的**相对轨道要素特征**（如半长轴之差 `Δa` 和相位角之差 `Δθ`），这些特征对于判断能量和追赶态势至关重要。
- **先进的智能体架构 (HRG - Hierarchical Recurrent Graph Network):** 
    ...
    - **态势感知模块 (Student Encoder):** 这是一个基于注意力机制的编码器，负责在每一时刻动态地融合多种信息源。可选择 `HAFN` (默认)、`LSTM` 或 `MLP` 架构。它融合：
    ...
    - **非对称训练范式 (Asymmetric Actor-Critic):** 
        ...
        - **评论家 (Critic):** 在训练阶段，Critic可以获取包含所有智能体真实状态（包括相对轨道要素）的“上帝视角”特权信息。这使其能对局势做出更准确的价值判断，从而更稳定、高效地指导Actor的学习。
...
- **鲁棒的课程学习 (Robust Curriculum Learning):**
    ...
    - **双维度难度调整 (Dual-Dimension Difficulty Adjustment):**
        ...
        - **战术与环境难度 (Tactical & Environmental Difficulty):**
            - **初始阵型课程 (Initial Formation Curriculum):** 训练过程中会动态调整追击方初始阵型的多样性。从固定的环形阵型（`ring`）开始，逐步过渡到更多样化、更具挑战性的随机阵型，如 `random`, `string` (一字长蛇), `pincer` (钳形)，迫使智能体学会应对各种初始态势。
            - **轨道扰动课程 (SMA Perturbation Curriculum):** 在训练中后期，为追逃双方的初始轨道半长轴引入随机扰动，增强环境的随机性，提升策略的鲁棒性。
    - **失败重试机制 (Failed-case Replay):** 训练中会维护一个失败场景的种子池。在每回合开始时，有一定概率（`replay_failed_prob`）从该池中抽取一个种子来复现之前的失败场景，强迫智能体针对性地学习如何解决困难问题。
...
│   └── new_models.py     # 定义HRG学生网络(HAFN/LSTM/MLP)和教师网络等模型结构
├── runs/                   # (训练后生成) 存放TensorBoard日志、课程进度日志和模型检查点
├── results/                # (评估后生成) 存放评估结果，如轨迹图
...
- `--student_model_type`: Student Encoder的架构 (`hafn`, `mlp`, `lstm`)。
- `--num_p`: 追击者的数量。
...
- `--save_media`: 为每个阵型的首次成功评估案例生成轨迹图。脚本会生成一个包含四种阵型相对轨迹的组合图 `trajectory_relative_combined.tiff`。
